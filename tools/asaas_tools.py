...
import httpx
import json
from config.config import ASAAS_API_KEY, ASAAS_SANDBOX_URL
...
async def _get_customer_by_cpf(cpf_cnpj: str, remotejid: str) -> str:
    """
    Internal function to search for a customer in Asaas by CPF/CNPJ.
    ...
    """
    if not all([ASAAS_SANDBOX_URL, ASAAS_API_KEY]):
        logger.error(f"[{remotejid}] Configurações do Asaas não estão completas")
        return json.dumps({"error": "Configurações do Asaas não estão completas"})
    try:
        async with httpx.AsyncClient() as client:
            response = await client.get(
                f"{ASAAS_SANDBOX_URL}/customers",
...
@retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=4, max=10))
async def _create_customer(cpf_cnpj: str, name: str = None, email: str = None, phone: str = None, remotejid: str = None) -> str:
    """
    Internal function to create a new customer in Asaas.
    ...
    existing_customer = await _get_customer_by_cpf(cleaned_cpf_cnpj, remotejid)
...
@retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=4, max=10))
async def _create_payment_link(customer_id: str, amount: float, description: str, remotejid: str = None) -> str:
    """
    Internal function to create a payment in Asaas for a customer using the /payments endpoint.
...
@function_tool
async def get_customer_by_cpf(cpf_cnpj: str, remotejid: str) -> str:
    """
    ...
    Returns:
        str: JSON string with customer data or error message.
    """
    return await _get_customer_by_cpf(cpf_cnpj, remotejid)

@function_tool
async def create_customer(cpf_cnpj: str, name: str = None, email: str = None, phone: str = None, remotejid: str = None) -> str:
    """
    ...
    Returns:
        str: JSON string with created customer data or error message.
    """
    return await _create_customer(cpf_cnpj, name, email, phone, remotejid)

@function_tool
async def create_payment_link(customer_id: str, amount: float, description: str, remotejid: str = None) -> str:
    """
    ...
    Returns:
        str: JSON string with payment data (including invoiceUrl) or error message.
    """
    return await _create_payment_link(customer_id, amount, description, remotejid)